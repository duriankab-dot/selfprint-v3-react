...
51. STORYTELLING ARCHITECTURE

Status: 6 Sep 2026 — Added as overlay layer on top of existing primitives. Not a new engine.

CORE PRINCIPLE

Storytelling is NOT a feature layer. It is a LENS through which every existing primitive is experienced.

SELFPRINT already has: Memory, Decisions, Twin, Evolution, Worlds, Today, Analysis.
The gap is: no thread connects them into a story the user can feel.

This section defines the Storytelling Layer — a narrative overlay that makes data feel like lived experience.

---

SEVEN STORY PRIMITIVES

These map onto existing data — no new database tables required.

| Primitive | Maps to (existing data) | Description |
|-----------|------------------------|-------------|
| Chapter | lifecycleStage / evolution_stage | A named period in the user's journey with the Twin |
| Story Beat | daily_brief / twin_memories | A single meaningful moment worth remembering |
| Narrative Hook | pattern from SICE + decision_logs | An opening question that pulls the user into deeper exploration |
| Question | onboarding responses + SICE patterns | A question the Twin holds that only the user can answer |
| Choice | decision_logs | A fork point the user reached — and which path they took |
| Consequence | follow_up results + evolution delta | What happened after a Choice — visible only when real data exists |
| Reveal | analysis + blueprint | A truth the Twin surfaces when enough data has been gathered |

---

THREE NARRATIVE LAYERS

Layer 1 — BIG STORY (across the entire journey)
- "Where you started. Where you are. Where you're going."
- Spans from onboarding → current evolution stage
- Rendered in: Twin Chat REVIEW mode · ME page · JOURNEY surface (G2)

Layer 2 — CURRENT CHAPTER (this week / this phase)
- The theme of the current period — named from dominant SICE patterns
- Rendered in: Today P0.2 · World detail Insight section

Layer 3 — MICRO STORY (Today)
- Today's single most meaningful Story Beat
- Rendered in: Today header · Twin greeting

---

FIVE STORY MODES (maps to §8 TWIN MODES)

| Mode | Trigger | Twin says |
|------|---------|-----------|
| REVEAL | Blueprint complete / major pattern confirmed | "I've noticed something consistent about how you handle [X]..." |
| EXPLORE | User enters a World | "This world connects to [pattern]. Want to see how?" |
...
RHYTHM TABLE — 7 Moments × Story Function

| Moment | Story Function |
|--------|---------------|
| Landing (first visit) | Hook: "What if you could finally understand why you make the decisions you do?" |
| Onboarding Q&A | Collection: building the raw material for the first Reveal |
| Core Awakening | Birth: the Twin holds everything collected so far |
| Twin Birth (first message) | First Reveal: NOT "Hi! Nice to meet you" — must reference real analysis |
| Today (returning user) | Micro Story: what happened since last visit that matters most |
| World entry | Chapter frame: how this world connects to the user's current pattern |
| Evolution stage change | Chapter close / open: the old chapter named, the new one introduced |

---

GUARDRAILS (hard limits — §44 safety rule applies)

1. NO FAKE STORY — if real data does not exist, show nothing. No "Your Twin sensed..." without actual SICE output.
2. NO GAMIFICATION — Narrative Hooks are not badges, streaks, or points. They are genuine questions.
3. NO PARALLEL MEMORY — the Storytelling Layer reads from existing twin_memories / decision_logs / daily_briefs. It does NOT create a separate memory system.
4. NO NEW INTELLIGENCE ENGINE — SICE generates all patterns. The Storytelling Layer only names and sequences them.
5. ONLY SHOW HOOK WHEN REAL DATA EXISTS — a Narrative Hook that references a pattern requires that pattern to be confirmed by SICE, not assumed.

---

IMPLEMENTATION MAPPING (which phases carry which story work)

| Track C Phase | Story work |
|---------------|------------|
| Phase 3 (Onboarding) | Narrative continuity: every screen answers "Why am I doing this?" |
| Phase 6 (Twin Birth) | First message must come from real analysis — never generic greeting |
| Phase 8 (Today) | P0.2 + daily Narrative Hook (real data: twin_memories / decision_logs / daily_briefs) |
| Phase 9 (Worlds) | World = "scene in the story" — each World shows Story / Pattern / Reflection / Decision |
| Phase 10/11 (Twin Modes) | Choice → Consequence surfacing + Memory Questions |
| G2 JOURNEY surface | Story History: P1 (Big Story) · P2 (Chapter archive) · records IA reserve from Today/Memory |
